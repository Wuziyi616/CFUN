...
class Config(object):
    ...
    IMAGE_MIN_SCALE = 0
    # Randomly crop or pad the image according to this ratio
    CROP_PAD_RATIO = 0.15
    # Randomly rotate the image according to this range of angle
    ROTATE_ANGLE = (-30, 31)
    ...
    def __init__(self, stage):
        ...
        self.IMAGE_META_SIZE = 1 + 3 + 4 + 6 + 1 + self.NUM_CLASSES
        self.STAGE = stage
        if stage == 'finetune':
            self.MINI_MASK_SHAPE = (64, 160, 160)
            self.MASK_SHAPE = (64, 160, 160)
        else:
            self.MINI_MASK_SHAPE = (32, 80, 80)
            self.MASK_SHAPE = (32, 80, 80)

        # Number of ROIs per image to feed to classifier/mask heads
        # Keep a positive:negative ratio according to ROI_POSITIVE_RATIO
        if stage == 'beginning':
            self.TRAIN_ROIS_PER_IMAGE = 50
            self.ROI_POSITIVE_RATIO = 0.33
        elif stage == 'together':
            self.TRAIN_ROIS_PER_IMAGE = 4
            self.ROI_POSITIVE_RATIO = 1.
        else:
            self.TRAIN_ROIS_PER_IMAGE = 4
            self.ROI_POSITIVE_RATIO = 1.
