...
def extract_bboxes(mask):
    ...
    mask: [depth, height, width].

    Returns: bbox array [(z1, y1, x1, z2, y2, x2)].
    """
    # Bounding box.
    ...
    boxes = np.array([z1, y1, x1, z2, y2, x2])
...
def extend_bbox(bbox, mask_shape, ratio=0.05):
    """Extend the bounding box by a given ratio in x, y, z dimension."""
    ...
    z2 = np.ceil(min(mask_shape[0], z2))
    y1 = np.floor(max(0, y1))
    y2 = np.ceil(min(mask_shape[1], y2))
    x1 = np.floor(max(0, x1))
    x2 = np.ceil(min(mask_shape[2], x2))
    bbox = np.array([z1, y1, x1, z2, y2, x2], dtype=np.int32)
...
def minimize_mask(bbox, mask, mini_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        m = skimage.transform.resize(m, mini_shape, order=0, mode="constant", preserve_range=True, anti_aliasing=False)
...
def expand_mask(bbox, mini_mask, image_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        h = y2 - y1
        w = x2 - x1
        m = skimage.transform.resize(m, (d, h, w), order=1, mode="constant", preserve_range=True, anti_aliasing=False)
        mask[z1:z2, y1:y2, x1:x2, i] = np.around(m).astype(np.int32)

    return mask


def unmold_mask(masks, bboxes, image_shape):
    ...
    # add up and divide
    full_mask_add = np.zeros((image_shape[1], image_shape[2], image_shape[3], masks.shape[-1]), dtype=np.float32)
    full_mask_count = np.zeros((image_shape[1], image_shape[2], image_shape[3], masks.shape[-1]), dtype=np.float32)
    masks = torch.from_numpy(masks).float().cuda()
    for i in range(masks.shape[0]):
        z1, y1, x1, z2, y2, x2 = bboxes[i]
        mask = masks[i]
        mask = mask.permute(3, 0, 1, 2).unsqueeze(0)  # convert to [N, D, H, W] and add up batch dimension
        ...
        full_mask_add[z1:z2, y1:y2, x1:x2, :] += mask
        full_mask_count[z1:z2, y1:y2, x1:x2, :] += 1.
    full_mask = full_mask_add / (full_mask_count + 1e-6)
